    ...
    <ul class="pagination justify-content-center">
        ...
        <?php for ($i = 1; $i <= $total_paginas; $i++) : ?>
            <li class="page-item <?= $i == $pagina_atual ? 'active' : '' ?>">
                <a class="page-link" href="?pagina=<?= $i ?>&search=<?= htmlspecialchars($search_query) ?>"><?= $i ?></a>
            </li>
        <?php endfor; ?>
        <!-- Botão da seta direita -->
        <li class="page-item">
            <a class="page-link" href="?pagina=<?= min($total_paginas, $pagina_atual + 1) ?>&search=<?= htmlspecialchars($search_query) ?>" aria-label="Next">
